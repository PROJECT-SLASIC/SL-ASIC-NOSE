module I2C_Slave#(parameter debounce = 3)(
    input clk,
    input rst,
    // I2C Ports
    input scl,
    ...
    assign read_1 = ((data_will_send && bit_count==5'd18) || (data_will_send && bit_count==5'd9) /*|| bit_change*/) ? (1'b1) : (1'b0);
    ...
    always @(posedge clk)begin
        if (rst)begin
            ...
            if((data_will_send && !received_one && (bit_count==5'd18)) || (data_will_send && !received_one && (bit_count==5'd9) && (restart || bit_change)) /*|| bit_change*/)begin
                i2c_data <= data_in;
                received_one <= 1'b1;
            end
                ...
                TLOW:begin
                    if(scl_reg)begin
                        if(counter >= TIME_TLOW)begin
                            bit_count <= (((restart || bit_change) && (bit_count==5'd9)) /*|| bit_change*/) ? (5'd19) : (bit_count + 1'b1);
//                            fsm_state <= THIGH;
                ...
                THIGH:begin
                    ...
                    if(scl_reg && !sda_reg && (bit_count==5'd9 || bit_count==5'd18))begin
                        sda_high_2 <= 1'b1;
                    end
                    else if (scl_reg && sda_reg && sda_high_2 && (bit_count==5'd9 || bit_count==5'd18))begin
                        sda_high_2 <= 1'b0;
                        fsm_state <= TSTO;
                        counter_reset <= 1'b1;
                        stop_received <= 1'b1;
                        ...
                        if((bit_count==5'd8) && half_ok)begin
                            if(temp_data[7:1]==I2C_SLAVE_ADDR)begin
                                ...
                                i2c_capt  <= (!temp_data[0]) ? ({1'b0, 8'hFF,1'b0,   8'hFF,1'b0,   8'hFF,1'b0,   1'b0}): // write
                                                               ({1'b0, 8'hFF,1'b0,   8'h00,1'b0,   8'h00,1'b0,   1'b0}); // read
                                data_will_send <= (!temp_data[0]) ? (1'b0) : (1'b1);
                                send_operation <= (!temp_data[0]) ? (send_operation) : (1'b1);
                                index_1 <= (!temp_data[0]) ? (index_1) : (read_operation);
                                bit_change <= (temp_data[0]);
                            end
                            else begin
                                nack_sended <= 1'b1;
                            end
                            ...
                            if((temp_data>=8'h40) && (temp_data<=8'h54))begin
                                ack_sended <= 1'b1;
                                index_1 <= temp_data;
                                read_operation <= temp_data;
                            end
                            else begin
                                nack_sended <= 1'b1;
                            end
                            ...
                            //bit_change <= (bit_change) ? (1'b0) : (bit_change);
                            ack_sended <= 1'b0;
                        end
